...
flowchart TD
    subgraph Data Pipeline
        raw[Raw CIFAR-10 Dataset] --> preproc[scripts/preprocess_data.py]
        preproc --> batches[data/processed/ Tensor Batches]
    end

    subgraph Training Pipeline
        batches --> train[src/training/train.py]
        train --> pth[models/trained/mobilenetv2.pth]
    end

    subgraph Export Pipeline
        pth --> export[scripts/export_model.py]
        export --> ts[models/exported/model.pt]
        export --> onnx[models/exported/model.onnx]
    end

    subgraph Serving System
        ts --> predictor[Unified Predictor]
        onnx --> predictor
        pth -.Fallback.-> predictor
        client[Client Requests] --> api[FastAPI / gRPC]
        api --> redis[(Redis Queue)]
        redis --> workers[Worker Pool / Batch Scheduler]
        workers --> predictor
    end
```
...
Here are the key performance metrics and scaling curves generated by the benchmarking suite:

### 1. Model Format Latency Comparison
![Model Format Latency](benchmarks/plots/fig_01_latency_formats.png)

### 2. Batch Latency & Throughput
![Batch Latency](benchmarks/plots/fig_02_batch_latency.png)
![Batch Throughput](benchmarks/plots/fig_03_batch_throughput.png)

### 3. Asynchronous Worker Scaling
![Worker Scaling](benchmarks/plots/fig_04_worker_scaling.png)

### 4. gRPC vs REST Protocol Comparison
![Protocol Comparison](benchmarks/plots/fig_05_protocol_comparison.png)

### 5. Device Performance (CPU vs MPS)
![Device Latency](benchmarks/plots/fig_06_device_latency.png)
![Device Throughput](benchmarks/plots/fig_07_device_throughput.png)
